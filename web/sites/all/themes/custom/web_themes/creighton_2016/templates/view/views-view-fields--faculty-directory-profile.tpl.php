	...
	<?php if (!empty($row->field_faculty_publications) || !empty($row->field_faculty_presentations)) {
		...
		if (!empty($row->field_faculty_publications)) {
			...
			foreach ($row->field_faculty_publications as $publications) {
				//make sure pubs aren't empty mmc 3.6.19
				if(!empty($publications)){
					foreach ($publications as $key => $pubcat) {
						foreach ($pubcat as $pcat) {
							//check if desc is there mmc 3.6.19, there will probably be more here
							//because these are filled by faculty and some may be missing.
							$tempPub = (!empty($pcat->Description)?trim($pcat->Description):'');
		...
		if (!empty($row->field_faculty_presentations)) {
			 ...
			 ?>
		<?php //make sure presentations array is not empty mmc 3/6/19
		if (count($publications_array) || !empty($presentations_array)) { ?>
		<h3 class="accordion-<?php print $tabNum; ?>">
		...
		<div id="accordion-<?php print $tabNum; ?>" class="acc-content">
          ...
          <?php endif; ?>
				...
				<?php //check if presentations array is empty mmc 3.6.19
				if (!empty($presentations_array)) : ?>
					<h3>Presentations</h3>
					<ul>
