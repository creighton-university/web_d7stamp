...
<?php if(!empty($row)) { ?>
	<?php if (!empty($row->field_faculty_first_name)) {
		...
		if (!empty($row->field_faculty_last_name)) {
			...
			$nameUrl = preg_replace("/\W+/", "-", $nameUrl);
		}
	...
	<div class="fac-staff-ldr-profile-content">
		<h3><a href="<?php print '/faculty-directory-profile/' . $row->nid . '/' . $nameUrl;; ?>"><?php print $fullname ?></a></h3>
...
//					}
				...
				&& count($row->field_fc_additional_pro_spclztns) > $special_num) { ?>
	...
	<td class="miscellaneous">
		...
		if (!empty($row->field_faculty_consult_honor_pat)) {
			foreach ($row->field_faculty_consult_honor_pat as $honor) {
				//check if honor description is there or not mmc 3.6.19
				print (!empty($honor->description)?$honor->description:'').' ';
			}
		}
		if (!empty($row->field_faculty_grants)) {
			foreach ($row->field_faculty_grants as $grant) {
				//check if grant description if there mmc 3.6.19
				print (!empty($grant->description)?$grant->description:'') . ' ';
			}
		}
		if (!empty($row->field_faculty_admin_roles_ldrshp)) {
			foreach ($row->field_faculty_admin_roles_ldrshp as $lead) {
				//this will probably need the same as the others		check if lead description if there mmc 3.6.19
				print $lead->{'activity-type'} . ', ' . (!empty($lead->office)?$lead->office:'') . ' ';
			}
		}
		?>
	</td>
